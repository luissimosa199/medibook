...
const AdsSwitch: React.FC = () => {
  ...
  const toggleAdsVisibilityMutation = useMutation(
    async (shouldShowAds: boolean) => {
      const response = await fetch(
        `/api/user/ads/?username=${encodeURIComponent(
          session?.user?.email as string
        )}`,
  ...
  return (
    <div className="mt-3 flex items-center justify-center">
      <label
        htmlFor="adSwitch"
        className="flex flex-col items-center gap-2 mr-4 text-lg cursor-pointer"
      >
        {showAds ? "Anuncios activados" : "Anuncios desactivados"}
