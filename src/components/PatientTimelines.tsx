...
const PatientTimelines: FunctionComponent<PatientTimelinesProps> = ({ username }) => {

    const fetchUserTimelines = async () => {
        const response = await fetch(`/api/pacientes/timelines/?username=${encodeURIComponent(username)}&page=0`, {
            method: 'GET'
        });
    ...
    const { data, isLoading, isError } = useQuery(["timelines", username], fetchUserTimelines)
