...
const TimelineForm: FunctionComponent = () => {
      ...
      try {
        ...
        if (previousData) {
          queryClient.setQueryData<{
            pages: TimelineFormInputs[][];
            pageParams: any[];
          }>(["timelines", session?.user?.email], previousData);
        }
        throw err;
      }
    } else {
  ...
  return (
    <form
      ...
      className="border-2 border-gray-300 flex flex-col gap-4 p-6 rounded max-w-[850px] mx-auto shadow-sm"
    >
      ...
      <FlexInputList
        ...
        placeholder="Agrega un link de YouTube"
        type="link"
        showState={false}
        primaryColor="red"
      />
