...
export default function Register() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (status === "unauthenticated") {
            setErrorMessage("Necesita ingresar con su cuenta para registrar un paciente")
            return
        }

        const response = await fetch('/api/pacientes/register', {
            method: 'POST',
            body: JSON.stringify({ email, name, details, tlf, doctor }),
            headers: {
    ...
    return (
        <div className="min-h-screen flex justify-center items-center bg-gray-100">
            <form className="bg-white p-8 rounded-lg shadow-md w-96" onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                    <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="tlf">
                        Número de contacto (opcional)
                    </label>
                    <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="tlf" type="tel" id="tlf" />
                </div>
                ...
                <div className="mb-4">
                    ...
                    <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="details" type="text" id="details" />
                </div>

                {session && session.user && <input name="doctor" type="text" id="doctor" readOnly hidden value={session?.user?.email as string} />}
